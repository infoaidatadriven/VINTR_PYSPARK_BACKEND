...
import collections
from MDM import editRulesetmdmglobal, getlistfromMDBglobal, CreateRulesetmdm_global, getprimarykey, getMasterDMview, \
    getMasterGOldenview, configureSource_mdm1, editconfiguredsource, getrules_mdm, getrules_mdmglobal, getlistfromMDB, \
    getlistfromMDB_vendorBased, CreateRulesetmdm, editRulesetmdm, checkSourceNameAvailability_Ndm
    ...
    if len(date_cols) != 0:
        for col_d in date_cols:
            df[col_d] = pd.to_datetime(df[col_d],
                                       dayfirst=True)  # .apply(lambda x: x.strftime(r'%d/%m/%Y')) #df[date_name_cols].apply(pd.to_datetime)
    if len(time_cols) != 0:
    ...
    if filesize > 15:
        ...
        if (file_ext == ".csv"):
            ...
            if type(data) == dict and len(list(data.keys())) != 0:
                msg1 = "JSON file contains the root key(s):- {}.Please reformat JSON file without any root keys.".format(
                    list(data.keys()))
...
def configureSource():
    ...
    EntryDataLineage(sourcedetails['templateSourcePath'], newSourceId, sourceDataName, newSourceId, str(datetime.now()),
                     "DQM", messagel, selectedColumns)
    EntryDataLineage(sourcedetails['templateSourcePath'], newSourceId, sourceDataName, newSourceId, str(datetime.now()),
                     "DQM", 'File storage', selectedColumns)
...
def EditconfigureSource():
    ...
    messagel = "File Received in DQM"
    EntryDataLineage(sourcedetails['templateSourcePath'], sourceId, sourceDataName, sourceId, str(datetime.now()),
                     "DQM", messagel, selectedColumns)
...
def DelayAnalysis():
    ...
    # else:
    #     df = pd.read_csv(sourcepath)
    cdecolumns = ["AIRLINE", "FLIGHT_NUMBER", "TAIL_NUMBER", "ORIGIN_AIRPORT", "DESTINATION_AIRPORT", "DEPARTURE_DELAY",
    ...
    for AIRLINE, frame in dfs:
        ...
        Dictconstraints1['TotalDelays'] = str(
            frame.query("ARRIVAL_DELAY >= 15")['ARRIVAL_DELAY'].sum())  # str((frame['ARRIVAL_DELAY']).sum())
        ...
        for ORIGIN_AIRPORT, framesub in framesum:
            ...
            Dictdet1['TotalDelays'] = str(
                framesub.query("ARRIVAL_DELAY >= 15")['ARRIVAL_DELAY'].sum())  # str((frame['ARRIVAL_DELAY']).sum())
...
def DelayAnalysisByAirport():
    ...
    for ORIGIN_AIRPORT, frame in dfs:
        ...
        Dictconstraints = {}
        Dictconstraints['NoOfDelays'] = str((frame['ARRIVAL_DELAY'] >= 15).sum())
        Dictconstraints['TotalDelays'] = str(
            frame.query("ARRIVAL_DELAY >= 15")['ARRIVAL_DELAY'].sum())  # str((frame['ARRIVAL_DELAY']).sum())
        ...
        for r in ruleset:
            ...
            for ru in r['rules']:
                if ru['rule'] == 'DataType':
                    ...
                    if (int(float(ru['value'])) == frame[col].str.len().min()) and (
                            int(float(ru['value'])) == frame[col].str.len().max()):
                        value.append(100)
                    else:
                        value.append(100 - ((((frame[col].str.len() != int(float(ru['value']))).sum()) / (
                            len(frame[col]))) * 100))
                        DNull = {}
                        DNull['column'] = col
                ...
                for item in uploadsObject:
                    ...
                    if multisrObj['multiSourceKey'] == multiSourceKey:
                        ...
                        if payloadDate.date() == actualDate.date():
                            content[
                                'errorMsg'] = 'The file for this multisource key is already uploaded for the current date'
                            content['errorflag'] = 'True'
    ...
    for obj in data['Analysis']:
        if isSourceValid(userName, userCategory, userDepartment, obj):
            ...
            for objldb in LDBDataList:
                ...
                dateLitnew = []
                dateLitnew = ((
                [((datetime.strptime(d['uploadDate'], "%Y-%m-%dT%H:%M:%S.%fZ"))).date() for d in objldb["uploads"] if
                 d["isCleanedSource"] != "YES"]))
                test_list = list(set(dateLitnew))
                for eachdate in test_list:
                    datelist = [d for d in objldb["uploads"] if (d["isCleanedSource"] != "YES" and (
                        datetime.strptime(d["uploadDate"], "%Y-%m-%dT%H:%M:%S.%fZ")).date() == eachdate)]
                    ...
                    for objldb1 in datelistnew:
                        ...
                        datelistCleanedNew = [d for d in objldb["uploads"] if (
                                    d["isCleanedSource"] == "YES" and d["uploadSourceId"] == objldb1["uploadId"])]
                        cleanList = []

                        uniqueSourcepath = [d['sourcePath'] for d in datelistCleanedNew if
                                            d["isCleanedSource"] == "YES"]
                        uniqueSourcepath1 = list(set(uniqueSourcepath))
                        newCleanedList = []
                        for eachsourceCleanedPath in uniqueSourcepath1:
                            cleanedListPerPath = [d for d in datelistCleanedNew if
                                                  d["sourcePath"] == eachsourceCleanedPath]
...
def getsourceDetails():
    ...
    for obj in sourcelist:
        ...
        for objldb in LDBDataList:
            ...
            dateLitnew = []
            dateLitnew = (
            ([((datetime.strptime(d['uploadDate'], "%Y-%m-%dT%H:%M:%S.%fZ")).date()) for d in objldb["uploads"]]))
            test_list = list(set(dateLitnew))
            for eachdate in test_list:
                datelist = [d for d in objldb["uploads"] if
                            (datetime.strptime(d["uploadDate"], "%Y-%m-%dT%H:%M:%S.%fZ")).date() == eachdate]
                # li= [d for d in datelist if (d["isCleanedSource"])=="YES"]
                dateuploadList = {}
                dateuploadList['uploadDate'] = eachdate
...
def LaunchAnalysisthread(sourceId, rulesetId, KeyNameList, uploadId):
    ...
    for keyItem in KeyNameList:
        ...
        p = mp.Process(target=LaunchAnalysisbyKeyNames,
                       args=(df2, keyItem, launchNewId, ruleset, cdecolumns, ReferenceObj, q))
        jobs.append(p)
        p.start()
    # for p in jobs:
    #    p.join()
...
def processSourcethread(sourceId, rulesetId, KeyNameList, uploadId, uploadDate, sourcedetails, settings):
    ...
    for keyItem in KeyNameList:
        ...
        p = mp.Process(target=processSourceByKeyName,
                       args=(df2, keyItem, launchNewId, ruleset, cdecolumns, ReferenceObj, uploadId, q))
        jobs.append(p)
        p.start()
    # for p in jobs:
    #    p.join()
    ...
    addAggResults(sourceId, sourcedetails, uploadId, uploadId, rulesetId, uploadDate, "0.00", DictFinal, sourcedataName,
                  sourcedataDesc, stype, settings)
    ...
    if AnalysisObj["dataOwner"] != "":
        userMsg = {}
        userMsg = getUsersFromownername(uploadDate, AnalysisObj["sourceDataName"], AnalysisObj["dataOwner"],
                                        json_object_user)

        userMsgList = userMsgList + (userMsg)

    if "dataUser" in AnalysisObj:
        userMsgList = userMsgList + getUsersFromCategory(json_object_user, AnalysisObj["dataUser"], DictFinal,
                                                         uploadDate, AnalysisObj["sourceDataName"],
                                                         AnalysisObj["dataOwner"])

    if "dataProcessingOwner" in AnalysisObj:
        userMsgList = userMsgList + getUsersFromCategory(json_object_user, AnalysisObj["dataProcessingOwner"],
                                                         DictFinal, uploadDate, AnalysisObj["sourceDataName"],
                                                         AnalysisObj["dataOwner"])

    if "dataSteward" in AnalysisObj:
        message = "The file is uploaded for " + uploadDate + " and the results are available in DashBoard for the source Named " + \
                  AnalysisObj["sourceDataName"]
        ...
        for obj in data['UserList']:
            if (cat['name'] in obj["userCategory"]):
                if (checkToleranceLimit(aggFinalResults, cat['tolerance'])):
                    ...
                    userMsg = {}
                    userMsg["user"] = owner
                    message = "The file which is uploaded for " + uploadDate + " and corresponding to the source Named " + sourceName + "doesn't meet with the tolerance of the usercategory " + \
                              cat['name']
                    userMsg["msg"] = message
                    userMsg["email"] = obj['email']
                    userMsg["Name"] = obj['name']
            ...
            for r in ruleset:
                ...
                for ru in r['rules']:
                    if ru['rule'] == 'DataType':
                        ...
                        if (int(float(ru['value'])) == frametbpcol[col].astype(str).str.len().min()) and (
                                int(float(ru['value'])) == frametbpcol[col].astype(str).str.len().max()):
                            value.append(100)
                        else:
                            cnttotal = frametbpcol[col].count()
                            if (cnttotal != 0):
                                value.append(100 - ((((frametbpcol[col].astype(str).str.len() != int(
                                    float(ru['value']))).sum()) / (len(frametbpcol[col]))) * 100))
                                DNull = {}
                                DNull['column'] = col
                                DNull['ruleType'] = 'Length'
                                DNull['rulevalue'] = ru['value']
                                DNull['ruleMismatchcount'] = str(
                                    (frametbpcol[col].astype(str).str.len() != int(float(ru['value']))).sum())
                        ...
                        if oper == 'equalto' or oper == 'Shouldbe':
                            ...
                            if (ComValue.isnumeric()):
                                ...
                                eloulierIndex = framenew.index[
                                    framenew[col].astype('float') != float(ComValue)].tolist()
            ...
            for r in ruleset:
                ...
                for ru in r['rules']:
                    if ru['rule'] == 'DataType':
                        ...
                        if (int(float(ru['value'])) == frame[col].astype(str).str.len().min()) and (
                                int(float(ru['value'])) == frame[col].astype(str).str.len().max()):
                            value.append(100)
                        else:
                            value.append(100 - ((((frame[col].astype(str).str.len() != int(
                                float(ru['value']))).sum()) / (len(frame[col]))) * 100))
                            DNull = {}
                            DNull['column'] = col
                            DNull['ruleType'] = 'Length'
                            DNull['rulevalue'] = ru['value']
                            DNull['ruleMismatchcount'] = str(
                                (frame[col].astype(str).str.len() != int(float(ru['value']))).sum())
                        ...
                        if oList[0] == '-':
                            ...
                            num_rows = len(framenew[(
                                        framenew[col1].astype('float') - framenew[col2].astype('float') == framenew[
                                    col].astype('float'))])
                            oulierIndex = framenew.index[(
                                        framenew[col1].astype('float') - framenew[col2].astype('float') != framenew[
                                    col].astype('float'))].tolist()
                        ...
                        if (reftotalCount - refCount > 0):
                            DNull = {}
                            DNull['column'] = col
                            DNull['rule'] = str(ru['rule']) + " " + str(fList[0]) + " " + str(oList[0]) + " " + str(
                                fList[1])
                            DNull['outLiers'] = str(reftotalCount - refCount)
...
def corelationrelationships():
    ...
    formulaList = []
    if 1 == 1:
        rel_dict = {'response_var': [], 'dependent_var1': [], 'dependent_var2': [], 'relationship': [], 'entries': [],
                    'percentOfEntries': []}
        ...
        for pair in dep_vars_pairwise_combinations:
            ...
            try:
                rel_true_array = np.array([True if (df_temp.loc[i, resp_var] == df_temp.loc[i, pair[0]] - df_temp.loc[
                    i, pair[1]]).any() else False for i in range(len(df_temp))])
            ...
            print(df_len_threshold)
            print('SUBTRACTION Relationship Identified in ~', 100 * np.round(rel_true_array.sum() / len(df_temp), 2),
                  '% of the entries')

            if (rel_true_array.sum() > df_len_threshold):
                print('Nr. of TRUE entries:', rel_true_array.sum(), '; \tDataFrame Size:', len(df_temp))
                rel_dict = {'response_var': [], 'dependent_var1': [], 'dependent_var2': [], 'relationship': [],
                            'entries': [], 'percentOfEntries': []}
                ...
                print('SUBTRACTION Relationship Identified in ~',
                      100 * np.round(rel_true_array.sum() / len(df_temp), 2), '% of the entries')
...
def GetCorrelationMatrix_v1(df, cols_data_type='mixed', method="None", type="", connectiondetails={}):
    ...
    if cols_data_type == 'numeric' and len(numeric_cols) == 0:  ## if there are no numeric columns re-assign defaults
        ...
        method = 'theils_u'
    if cols_data_type == 'categorical' and len(
            cat_cols) == 0:  ## if there are no categorical columns re-assign defaults
    ...
    if cols_data_type == 'categorical' or len(numeric_cols) == 0:
        ...
        if method == 'theils_u':
            ...
            corr_matrix = compute_associations(df_clean, nominal_columns='all', theil_u=True,
                                               nan_strategy='drop_samples')
        elif method == 'cramers_v':
            print('Computing Categorical Correlation Using Cramers_V method')
            corr_matrix = compute_associations(df_clean, nominal_columns='all', theil_u=False,
                                               nan_strategy='drop_samples')
    if cols_data_type == 'mixed':
        mixed_cols = numeric_cols + cat_cols
        print("Mixed Cols:", mixed_cols)
        corr_matrix = compute_associations(df[mixed_cols], nominal_columns='auto', theil_u=False,
                                           bias_correction=False)  # , nan_strategy = 'drop_samples')
    ...
    try:
        ...
        if 1 == 1:
            ...
            for pair in dep_vars_pairwise_combinations:
                ...
                df_len_threshold = int(
                    len(df_temp) / 1.65)  # Threshold for valid entries set at 60% of the dataframse size

                # Check if the dependant variables have a subtractive relationship for every entry in the dataframe
                try:
                    addrel_true_array = np.array([True if (df_temp.loc[i, resp_var] == sum(
                        [df_temp.loc[i, pair[j]] for j in range(nr_combinations)])).any() else False for i in
                                                  range(len(df_temp))])
                    if nr_combinations == 2:
                        rel_true_array = np.array([True if (
                                    df_temp.loc[i, resp_var] == df_temp.loc[i, pair[0]] - df_temp.loc[
                                i, pair[1]]).any() else False for i in range(len(df_temp))])
                        if rel_true_array.sum() == 0:
                            # change the pair if there is no subtraction relationship with current pair (a,b) -> (b,a)
                            rel_true_array = np.array([True if (
                                        df_temp.loc[i, resp_var] == df_temp.loc[i, pair[1]] - df_temp.loc[
                                    i, pair[0]]).any() else False for i in range(len(df_temp))])
                            rev_combination_found = True
                        mulrel_true_array = np.array([True if (
                                    df_temp.loc[i, resp_var] == df_temp.loc[i, pair[0]] * df_temp.loc[
                                i, pair[1]]).any() else False for i in range(len(df_temp))])
                        divrel_true_array = np.array([True if (df_temp.loc[i, resp_var] == round(
                            (df_temp.loc[i, pair[0]] / df_temp.loc[i, pair[1]]) * 100, 2)).any() or (
                                                                          df_temp.loc[i, resp_var] == round((
                                                                                                                    df_temp.loc[
                                                                                                                            i,
                                                                                                                            pair[
                                                                                                                                1]] /
    ...
    try:
        ...
        if 1 == 1:
            ...
            for pair in dep_vars_pairwise_combinations:
                df_temp = df_clean[[resp_var, pair[0], pair[1]]]
                df_len_threshold = int(
                    len(df_temp) / 1.6)  # Threshold for valid entries set at 60% of the dataframse size
                # Check if the dependant variables have a subtractive relationship for every entry in the dataframe
                rel_true_array = np.array([True if (df_temp.loc[i, resp_var] == df_temp.loc[i, pair[0]] - df_temp.loc[
                    i, pair[1]]).any() else False for i in range(len(df_temp))])
                addrel_true_array = np.array([True if (
                        df_temp.loc[i, resp_var] == df_temp.loc[i, pair[0]] + df_temp.loc[
                        i, pair[1]]).any() else False for i in range(len(df_temp))])
                mulrel_true_array = np.array([True if (
                            df_temp.loc[i, resp_var] == df_temp.loc[i, pair[0]] * df_temp.loc[
                ...
                divrel_true_array = np.array([True if (df_temp.loc[i, resp_var] == round(
                    (df_temp.loc[i, pair[0]] / df_temp.loc[i, pair[1]]) * 100, 2)).any() or (
                                                                  df_temp.loc[i, resp_var] == round(
                                                              (df_temp.loc[i, pair[1]] / df_temp.loc[i, pair[0]]) * 100,
                                                              2)).any() else False for i in range(len(df_temp))])
...
def profileAnalysisRef(sourcepath, df):
    ...
    df_meta_inferred.columns = ["column_name", "inferred_dtype"]
    df_meta = df_meta.merge(df_meta_inferred, on="column_name", how="left")
    conFeat = df_meta[df_meta.dtype == "int64"].column_name.tolist() + df_meta[
        df_meta.dtype == "float64"].column_name.tolist()
    print("conFeat:", conFeat)
    dateFeat = df_meta[df_meta.dtype == "datetime64[ns]"].column_name.tolist() + df_meta[
        df_meta.dtype == "datetime"].column_name.tolist()
    print("DateTime Features:", dateFeat)
    catFeat = df_meta[df_meta.dtype == "object"].column_name.tolist()
    print("catFeat:", catFeat)
    ListResult = []
    ...
    for k in selectedColumns:
        ...
        if k in catFeat:
            if (df[k].nunique() != 0):
                df_describe_categorical = forStringType([k], df)
                if (str(mask_val["unique_values"]).contains("L") for mask_val in
                    df_describe_categorical['maskAnalysis']) and (
                        df_describe_categorical['attributeSummary']['dataType'] == "Numeric"):
                    print("\n Column for Outlier_Len_Analysis:", k, "DataType:",
                          df_describe_categorical['attributeSummary']['dataType'])
                    df_describe_categorical['outlier_len_statistics'] = outlier_len_stats_numeric_cols(
                        df_describe_categorical['maskAnalysis'])
                ResMatrix = sorted_corr_matrix_per_col(CateoricalCorrMatrix, k)
                # print("\n df_describe_categorical:",df_describe_categorical)
                df_describe_categorical['correlationSummary'] = ResMatrix
                ListResult.append(df_describe_categorical)
...
def forDateTimeType(col, df):
    ...
    profileDict["attributeSummary"] = {"records": total_count, "dataType": "DateTime", "null_records": null_count,
                                       'outliers': null_count, 'duplicates': null_count, 'invalid': null_count}
...
def NumericStatistics(conFeat, data):
    conHead = ['Count', 'NullCount', 'UniqueValuesCount', 'MinimumValue', 'MeanValue', 'MedianValue', 'MaximumValue',
               'Std_Dev', 'minLength', 'MaxLength', 'MeanLength', 'MedianLength']
...
def stringStatistics(catFeat, data):
    ...
    for col in columns:
        ...
        isAlpha[index] = columns[col].str.isalpha().sum()
        isNum[index] = columns[col].str.isnumeric().sum()
        isAlNum[index] = (columns[col].str.isalnum().sum()) - (columns[col].str.isalpha().sum()) - (
            columns[col].str.isnumeric().sum())
...
def addAggResults(sourceId, sourcedetails, launchId, uploadId, rulesetId, uploadDate, uploadTime, aggResults,
                  sourcedataName, sourcedataDesc, stype, settings):
...
def GetDB_Collections_BackEnd(
        client_url="mongodb+srv://admin:admin@cluster1.zyygq.mongodb.net/admin?retryWrites=false&w=majority"):
    '''Get a dictionary of all databases in a cluster, names of collections and field names in each collection'''
    client = MongoClient(client_url)
    dbs_list = [x for x in client.list_database_names() if x not in ['admin', 'local']]
    db_colls_dict = {}
                ...
                for f in fields:
                    ...
                    if type(query) == str:
                        if "matchcase" in content.keys():
                            res = list(client[db][c].find({f: query},
                                                          {'_id': 0}))  ##Avoid returning Object ID as it creates Error
                        else:
...
def MongoDB_Collection_Save():
    ...
    if client_url != "":
        ...
        log_old = data['SavedFilesLog']
        new_log_data = {
            db: {col: {"type": "User_LocalData", "sourceDataName": source_data_name, "outputpath": output_path}}}
        log_old.append(new_log_data)
        data['SavedFilesLog'] = log_old
        json.dump(data, open("mongodb_data/mongoDB_SaveLog.json", "w"), default=str)
    print("Saving Cleaned data file at {}".format(output_path))
...
def MongoDB_Collection_Copy():
    ...
    if col in existing_collection_names:
        return {'errorMsg': "Collection Name already exists. Please input alternative collection name!",
                "errorCode": '110', 'errorflag': 'True'}
            ...
            if file_ext in app.config['UPLOAD_EXTENSIONS']:
                ...
                try:
                    ...
                    if any('Unnamed' in col for col in df.columns.values.tolist()):
                        ...
                        print(error_msg)
                        raise pd.errors.ParserError  ## ['.','$','*',':','|','?']
                    if (col.str.contains('.|$|*|:|?|<|>', case=False, regex=True) for col in
...
def DomainType_Identifier():
    ...
    for col in df_source_cols:
        ...
        for key in ref_cols_dict.keys():
            ...
            for c in split_text:
                if c in ref_cols:
                    print("\tColumn '{}' containing '{}' - Found in Domain '{}' containing {}".format(col, c, key,
                                                                                                      ref_cols))
...
def GetSourceResults():
    ...
    for obj in data['AggResults']:
        ...
        if isSourceValid(userName, userCategory, userDepartment, obj):
            ...
            datelist = []
            datelist = (
            ([((datetime.strptime(d['uploadDate'], "%Y-%m-%dT%H:%M:%S.%fZ")).date()) for d in obj["results"]]))
            test_list = list(set(datelist))
            for objdate in test_list:
                LDBDataList = [d for d in obj["results"] if
                               (datetime.strptime(d["uploadDate"], "%Y-%m-%dT%H:%M:%S.%fZ")).date() == objdate]
                obj1 = {}
...
def rulesWithRefthreadnew():
    ...
    # 06/06/2022- Thiru #51 & #57 (Ruleset formula detection issues)
    for k in selectedColumns:
        p = mp.Process(target=rulesforSingleColumn, args=(
        df, k, conFeat, catFeat, dateFeat, NumericCorrMatrix, CateoricalCorrMatrix, refColumns, df_corr_sort, q))
        jobs.append(p)
        p.start()
        print('starting')
    print('completed')
...
def rulesWithtest():
    ...
    rules = []
    selectedColumns = ['PassengerId', 'Pclass', 'Age', 'SibSp', 'Parch', 'Fare', 'Name', 'Sex', 'Ticket', 'Cabin',
                       'Embarked']
    df_meta = pd.DataFrame(df.dtypes).reset_index()
    df_meta.columns = ["column_name", "dtype"]
    df_meta_inferred = pd.DataFrame(df.infer_objects().dtypes).reset_index()
...
def rulesforSingleColumn(df, selectedColumn, conFeat, catFeat, dateFeat, NumericCorrMatrix, CateoricalCorrMatrix,
                         refColumns, df_corr_sort, q):
    content = request.get_json()
    sourcepath = content['sourcepath']
    connectiondetails = {}
    typeofcon = ""
    ...
    if (ruleConfigforMultiCols == "") and (len(ruleConfigforMultiCols) < 3):
        return {'errorMsg': "Atleast 3 columns should be selected for creating formula or ruleset!", "errorCode": '110',
                'errorflag': 'True'}
            ...
            if is_numeric_dtype(df[k]):
                ...
                if df[k].nunique() > 1:
                    ...
                    if ruleConfigfor2Cols != "":
                        ...
                        nr_combinations = 2
                        dep_vars_pairwise_combinations = list(combinations(dependent_features, nr_combinations))
                        add_new_rule = getRelationshipForRules(df_rule, k, nr_combinations,
                                                               dep_vars_pairwise_combinations)
                        if add_new_rule:
...
def rulesforSingleColumnTest(df, selectedColumn, conFeat, catFeat, dateFeat, NumericCorrMatrix, CateoricalCorrMatrix,
                             refColumns, df_corr_sort):
    content = request.get_json()
    sourcepath = content['sourcepath']
    connectiondetails = {}
    typeofcon = ""
...
def getdistinct(keyname):
    ...
    for eachvalue in data_dict:
        dictList = {}
        df = pd.read_sql_query('SELECT *  FROM tblflights  where ' + keyname + '="' + eachvalue + '"  limit 5000',
                               csv_database)
...
def localRefCluster_query():
    ...
    with sqlite3.connect('csv_database_ref.db') as conn:
        ...
        for eachtable in listofDictTables:
            ...
            fieldName = chunks[1]
            if matchcase == "YES":
                df = pd.read_sql_query('SELECT * FROM [' + tablename + '] where [' + fieldName + '] ="' + query + '"',
                                       csv_database)
            else:
                df = pd.read_sql_query(
                    'SELECT * FROM [' + tablename + '] where lower([' + fieldName + ']) LIKE "%' + query + '%"',
                    csv_database)
            df.drop('index', axis=1, inplace=True)
            ...
            if file_ext in app.config['UPLOAD_EXTENSIONS']:
                ...
                try:
                    ...
                    if len(compare_cols) > 0:
                        return {
                            'errorMsg': "At least one column name is matching with the available columns in the database",
                            "errorCode": '110', 'errorflag': 'True'}
                except (pd.errors.ParserError, pd.errors.EmptyDataError):
                    content = {}
                    content['errorMsg'] = 'Data Parsing Failed. {}'.format(error_msg)
            ...
            if common_data:
                ...
                for col in ref_df.columns:
                    ...
                    if col_common_data:
                        ...
                        source_file_col_name = [col for col in source_columns if
                                                any(val in col_common_data for val in source_df[col])]
                        common_data_dict = {
                        ...
                        jaccard_index = len(common_data_set.intersection(ref_data_set)) / len(
                            common_data_set.union(ref_data_set))
